...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    minHeight: 600,
    title: 'Mingly',
    // macOS: frameless inset title bar with custom traffic light position
    // Windows/Linux: standard OS-native frame with title bar
    ...(isMac
    ...
    webPreferences: {
      ...
      sandbox: true, // Enabled: safeStorage replaces keytar
      webSecurity: true,
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          `default-src 'self'; script-src 'self'${isDev ? " 'unsafe-inline'" : ''}; style-src 'self' 'unsafe-inline'; connect-src ${connectSrc}; img-src 'self' data:; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'`
        ]
      }
    })
  })
