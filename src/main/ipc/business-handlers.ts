...
export function registerBusinessHandlers(): void {
  ...
  // ========================================
  // Tracking & Analytics — Team+ tier
  // ========================================

  wrapHandler(IPC_CHANNELS.TRACKING_GET_SUMMARY, (fromMs?: number, toMs?: number) => { requireFeature('usage_tracking'); return { success: true, summary: trackingEngine.getSummary(fromMs, toMs) } })
  wrapHandler(IPC_CHANNELS.TRACKING_GET_DAILY_USAGE, (days?: number) => { requireFeature('usage_tracking'); return { success: true, dailyUsage: trackingEngine.getDailyUsage(days) } })
  wrapHandler(IPC_CHANNELS.TRACKING_GET_RECENT_EVENTS, (limit?: number) => { requireFeature('usage_tracking'); return { success: true, events: trackingEngine.getRecentEvents(limit) } })
  ...
  // ========================================
  // Deployment / Server + Client — Enterprise tier
  // ========================================

  wrapHandler(IPC_CHANNELS.DEPLOYMENT_GET_CONFIG, () => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, config: deploymentManager.getConfig() } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_UPDATE_CONFIG, (config: any) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, config: deploymentManager.updateConfig(config) } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_GET_STATUS, () => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, status: deploymentManager.getStatus() } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_START_SERVER, async (configOverride?: any) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return deploymentManager.startServer(configOverride) })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_STOP_SERVER, async () => { requireFeature('on_premise'); requirePermission('deployment.manage'); return deploymentManager.stopServer() })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_ADD_REMOTE, (server: any) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, server: deploymentManager.addRemoteServer(server) } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_REMOVE_REMOTE, (serverId: string) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, removed: deploymentManager.removeRemoteServer(serverId) } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_LIST_REMOTES, () => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, servers: deploymentManager.getRemoteServers() } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_CHECK_REMOTE, async (serverId: string) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, ...(await deploymentManager.checkRemoteServer(serverId)) } })
  wrapHandler(IPC_CHANNELS.DEPLOYMENT_DISCOVER_SERVERS, async (networkRange?: string) => { requireFeature('on_premise'); requirePermission('deployment.manage'); return { success: true, servers: await deploymentManager.discoverServers(networkRange) } })
  ...
  wrapHandler(IPC_CHANNELS.ORCHESTRATOR_APPROVE, (proposalId: string) => { requireFeature('agents'); return { success: orchestrator.approveProposal(proposalId) } })
  wrapHandler(IPC_CHANNELS.ORCHESTRATOR_DENY, (proposalId: string) => { requireFeature('agents'); return { success: orchestrator.denyProposal(proposalId) } })
  wrapHandler(IPC_CHANNELS.ORCHESTRATOR_EXECUTE, async (proposalId: string) => { requireFeature('agents'); const r = await orchestrator.executeDelegation(proposalId); return { success: !!r, result: r } })
